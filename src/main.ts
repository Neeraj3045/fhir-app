...
async function bootstrap() {
  ...
  app.setGlobalPrefix(process.env.GLOBAL_PREFIX, {
    exclude: [{ path: process.env.GLOBAL_PREFIX, method: RequestMethod.GET }],
  });
    ...
    new ValidationPipe({
      ...
      exceptionFactory: (errors: ValidationError[]) => {
        errors.map(function (error) {
          let jsonData = {};
          jsonData = `${Object.values(error.constraints)}`;
          return jsonData;
        });
      },
    }),
  ...
  const config = new DocumentBuilder()
    ...
    .setContact(
      ...
      process.env.SWAGGER_CONTACT_WEBSITE,
      process.env.SWAGGER_CONTACT_EMAIL,
    )
    .setTermsOfService(process.env.SWAGGER_TERMS_OF_SERVICES)
    .setExternalDoc(
      process.env.SWAGGER_EXTERNAL_DOC,
      process.env.SWAGGER_EXTERNAL_DOC_URL,
    )
    .setLicense(
      process.env.SWAGGER_LICENSE_NAME,
